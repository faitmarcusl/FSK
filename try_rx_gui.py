...
def start(in_file, base_freq, offset, run, progress, orig_plot, fsk_plot):
    # Start the FSK reception process
    audio = in_file.get()
    ...
    binary_data, audio_samples, sampling_rate = run(audio, output, base_freq, offset)
    progress.set(100)
    # Plot the received and demodulated waveforms
    plot(audio, orig_plot, "Received Modulated Waveform", binary_data, sampling_rate, modulated=False)
    plot(audio_samples, fsk_plot, "Demodulated Audio Waveform", binary_data, sampling_rate, modulated=True)
...
def display(run):
    # Launch the GUI for FSK demodulation receiver
    root = tk.Tk()
    ...
    tk.Label(root, text="Input Modulated Audio File:").grid(row=0, column=0, padx=10, pady=5)
    ...
    tk.Button(root, text="Demodulate", command=lambda: start(in_file, base_freq, offset, run, progress, orig_plot, fsk_plot)).grid(row=4, column=0, columnspan=3, padx=10, pady=10)
    ...
    # Add canvas for plotting demodulated waveform
    fig, ax = plt.subplots(figsize=(8, 4))
